...
const Detail = () =>
{
  const { formattedCheckinDate, formattedCheckoutDate, guest } = useSelector( ( state ) => state.reserve.reserveData );
  const { id } = useParams();
  ...
  return (
    <div className="container mx-auto px-2">
      ...
      <div className="">
        ...
        <div onClick={() => navigate('/')} className=" flex justify-start border border-b-2 py-5 items-center xl:hidden">
      ...
      <div className=" mt-[20px] xl:mt-[180px] grid gap-5 grid-cols-1 2xl:grid-cols-3">
        <div className="col-span-2">
          ...
          <div className="my-5">
            <img src={ image } className=" rounded-sm xl:rounded-2xl h-[300px] lg:h-[350px] xl:h-[450px] w-full" />
          </div>
          <h1 className="text-gray-800 text-[20px] lg:text-[30px] xl:text-[35px] font-semibold">{ title }</h1>
          <div className="flex gap-3">
            <span className=" text-[14px] lg:text-[20p] xl:text-[22px] text-gray-700">{ guests } guests</span>
            ...
            <span className=" text-[14px] md:text-[20p] xl:text-[22px] text-gray-700">{ beds } beds</span>
          </div>
          ...
          <hr className="my-10" />
          <div>
            <h1 className=" text-[17px] lg:text-[22px] xl:text-[28px] my-4 ">What you'll sleep</h1>
            <div className="flex flex-wrap gap-5">
              <div className="border-2 rounded-2xl text-center p-3 lg:p-6 xl:p-10">
                ...
                <div className=" text-[15px] lg:text-[18px] xl:text-[22px] mt-3 font-bold">Bedroom { bedrooms }</div>
              </div>
              <div className="border-2 rounded-2xl text-center p-3 lg:p-6 xl:p-10">
                <LiaBedSolid className=" text-[20px] lg:text-[25px] xl:text-[30px] inline" />
                <div className=" text-[15px] lg:text-[18px] xl:text-[22px] mt-3 font-bold">Beds { beds }</div>
              </div>
              <div className="border-2 rounded-2xl text-center p-3 lg:p-6 xl:p-10">
                <MdOutlineBathroom className=" text-[20px] lg:text-[25px] xl:text-[30px] inline" />
                <div className=" text-[15px] lg:text-[18px] xl:text-[22px] mt-3 font-bold">Bathrooms { bathrooms }</div>
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="border-2 rounded-2xl mt-5  ">
                <div className="grid grid-cols-2 ">
                  <button className="border-r-2 p-3">
                    <div className="text-left">
                      ...
                      <h1 className="uppercase text-gray-700 text-sm">
                        { formattedCheckinDate === 'Invalid Date' ? 'Add Date' : formattedCheckinDate }
                      </h1>
                    </div>
                  </button>
                  <button className="p-3">
                    <div className="text-left">
                      ...
                      <h1 className="uppercase text-gray-700 text-sm">
                        { formattedCheckoutDate === 'Invalid Date' ? 'Add Date' : formattedCheckoutDate }
                      </h1>
                    </div>
                  </button>
                </div>
                <div className="flex justify-between items-center border-t-2 p-4">
                  ...
                  <div className="flex gap-3 items-center">
                    ...
                    <span className="text-[20px]">{ guest ? guest : 'Add Guest' }</span>
                    <button className="rounded-full border-2 border-gray-500 text-gray-500 w-8 h-8 cursor-pointer">
                      -
                    </button>
                  </div>
      ...
      <div className=" fixed bottom-0 z-[1998] bg-white w-full flex justify-between items-center xl:hidden px-10 py-5">
        <div className="">
          <h1 className=" text-[20px] lg:text-[25px] xl:text-[30px] font-bold">
            ${ price } <span className=" text-[15px] md:text-[18px] lg:text-[22px] xl:text-[25px] font-normal">night</span>{ " " }
          </h1>
        </div>
